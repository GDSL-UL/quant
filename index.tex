...
\begin{Highlighting}[]
\NormalTok{APS }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{count}\NormalTok{(Hhold\_Size) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{arrange}\NormalTok{(}\FunctionTok{desc}\NormalTok{(n))}
\end{Highlighting}
\end{Shaded}
...
Household with 2 persons account for 86,065 households in the data.
...
NA values, ensuring that NA values are removed. Find the mean value for
Net\_Pay and be sure to remove NA values. Write code to show this.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{( APS}\SpecialCharTok{$}\NormalTok{Net\_Pay,  }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The North West's Gini coefficient (0.27) is slightly lower than the
overall value (0.28). This means income in the North West is distributed
slightly more evenly than across the UK as a whole. In practical terms,
there is marginally less disparity between lower-income and
higher-income individuals in the North West compared with the national
picture.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{min}\NormalTok{(APS}\SpecialCharTok{$}\NormalTok{Net\_Pay, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
This could be people working very few hours such as students or be due
to temporary or irregular employment.
...
The most important geographical divide is London vs the rest of the UK,
followed by a broader South East vs other regions pattern. While there
is also evidence of a North--South divide, it is less sharp and appears
to be driven largely by proximity to London rather than latitude alone.
This reflects the strong spatial concentration of high-value economic
activity in London and surrounding regions, which contributes to higher
wages and reinforces regional inequalities in income across the UK.
...
Gross pay represents total earnings before deductions, while net pay
represents take-home income after deductions. Since deductions are
generally a proportion of gross income, net pay increases strongly as
gross pay increases.
...
`<!-- quarto-file-metadata: eyJyZXNvdXJjZURpciI6ImxhYnMifQ== -->`{=html}

```{=html}
<!-- quarto-file-metadata: eyJyZXNvdXJjZURpciI6ImxhYnMiLCJib29rSXRlbVR5cGUiOiJjaGFwdGVyIiwiYm9va0l0ZW1OdW1iZXIiOjcsImJvb2tJdGVtRmlsZSI6ImxhYnMvTW9jayBleGFtX0JfU29sdXRpb25zLnFtZCIsImJvb2tJdGVtRGVwdGgiOjB9 -->
```

# Mock questions 

```````{.quarto-title-block template='C:\Program Files\RStudio\resources\app\bin\quarto\share\projects\book\pandoc\title-block.md'}
---
title: Mock questions
...
\chapter{Mock Test}\label{mock-test}

\phantomsection\label{Preface}
\textbf{Before starting it's good to get some of the preliminary steps
taken care of first. In this case, we first download the data, and then
clear the workspace to get rid of older variables that may be in our
environment area since last working with RStudio. Finally, we import the
libraries that we will be using.}

\section{Data download}\label{data-download}

Some of the datasets on Canvas have been updated, so please re-download
the data.

\begin{itemize}
\item
  \emph{world\_value\_survey} - Located in Week 3
\item
  \emph{LAD December 2021 EW BUC.geojson} - Located in Week 4
\item
  \emph{Census\_2021\_Wards.csv} - Located in Week 4
\item
  \emph{Census\_2021\_Districts.csv} - Located in Week 4
\end{itemize}

\section{Clear the workspace}\label{clear-the-workspace}

This is important because you could be using an older dataset that is
incorrect if you don't do this.
...
These are the libraries that will use be using for the mock test. The
specific libraries to be loaded will depend on the specific task to be
completed. However, these are the most common ones.
...
\textbf{This mock exam consists of a series of 25 questions. Many of
these questions are from the labs, so be sure to go through these before
the final exam. Your final exam will have 20 questions and will be
multiple choice. Unless told otherwise in the exam, you will be expected
to write code (i.e., copy and paste from labs and modify accordingly) to
derive results, which you will then use to determine to correct answer
from a set of multiple choice options. You are not expected to write
code from scratch.}

\section{Question 1}\label{question-1}

Write code to load the \emph{world\_value\_survey.csv} dataset into
RStudio. How many \textbf{rows} and \textbf{columns} does this dataset
have?~
...
\begin{Highlighting}[]
\CommentTok{\# use read.csv to load a CSV file}
\NormalTok{world\_value }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"world\_value\_survey.csv"}\NormalTok{,}\AttributeTok{stringsAsFactors =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# dimension of a dataframe}
\FunctionTok{dim}\NormalTok{(world\_value)}
\end{Highlighting}
\end{Shaded}
...
Use \texttt{dim()} to get the dimension of a dataframe. The answer is
153009 rows and 49 columns.

\section{Question 2}\label{question-2}

Write code to load the \emph{Census\_2021\_Wards.csv} dataset into
RStudio. Calculate the total \textbf{sum} of the variable \emph{Worker}
across all wards in the dataset.~

Solution

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# use read.csv to load the CSV file}
\NormalTok{wards }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"Census\_2021\_Wards.csv"}\NormalTok{,}\AttributeTok{stringsAsFactors =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\#sum up the variable}
\FunctionTok{sum}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Worker)}
\end{Highlighting}
\end{Shaded}
...
Use \texttt{sum()} to get the total counts. The answer is 127,773,832
workers in all wards.

\section{Question 3}\label{question-3}

Find the \textbf{min} value of variable \emph{Density} in the Wards
dataset \textbf{without code.}

Solution

Click the dataset name \texttt{"wards"} in the Environment pane to open
the Data Viewer, then click the \texttt{Density} column header to sort
in ascending order and the top value is the min value of variable
\texttt{Density}.Click on the data table containing the variable in the
Environment pane to the right of the screen in RStudio. A new window
should open up with the table. Next, navigate to the the specific
variable in the table and click on it's name. Usually, clicking on the
name the first time will sort the variable in ascending order, and
clicking again will sort the variable in descending order. Just beware
of the ordering. In ascending order the smallest value will be at the
top of the column. It's similar to operation in Microsoft Excel.

\section{Question 4}\label{question-4}

Write code to identify the ward with the \textbf{min} value for the for
the variable \emph{Mean\_Age} using the Census\_2021\_Wards dataset.
...
\CommentTok{\#slice the dataframe by the row with min value in the variable}
\NormalTok{wards }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{slice}\NormalTok{(}\FunctionTok{which.min}\NormalTok{(Mean\_Age))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
     WD21CD         Ward_name District      County     Region Super_Region
...
1     11.2069            546         5.045278
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#or sort the dataframe by the variable, return the top 1}
\NormalTok{wards }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{arrange}\NormalTok{(Mean\_Age) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{head}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The above provides two ways to get the row with min value in
\texttt{Mean\_Age} from dataset \texttt{wards}. The answer is Holywell
in Oxford.

\section{Question 5}\label{question-5}

Write code to compute the \textbf{overall proportion} of workers who
work from home (variable: Work\_from\_Home) to the total works
(variable: Worker) in the \emph{Census\_2021\_Wards} dataset.
...
\CommentTok{\# Compute the overall proportion}
\FunctionTok{sum}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Work\_from\_Home) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Worker)}
\end{Highlighting}
\end{Shaded}
...
We first calculate the sum of variable \texttt{Work\_from\_Home}, then
divide it by the sum of variable \texttt{Worker}. The answer is 31.2\%

\section{Question 6}\label{question-6}

Write code to generate a statistical summary of the variable
\emph{Social\_Rented} using the \emph{Census\_2021\_Wards.csv} dataset.
From the output, identify and report the \textbf{mean} value of
\emph{Social\_Rented}.~
...
\CommentTok{\# get a statistical summary of one variable}
\FunctionTok{summary}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Social\_Rented)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 0.4677  7.9926 12.6332 15.2696 19.9193 75.3656 
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#for mean only}
\FunctionTok{mean}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Social\_Rented)}
\end{Highlighting}
\end{Shaded}
...
Use \texttt{summary()} to get a statistical summary of one variable. Use
\texttt{mean()}, \texttt{median()}, \texttt{min()}, \texttt{max()} to
get specific statistical results. Bonus: try \texttt{fivenum()} to see
what you can get? Learn it by using \texttt{?fivenum} yourself. The
answer of this one is 15.27.

\section{Question 7}\label{question-7}

Write code to produce a frequency table of the variable \emph{religion}
using the \emph{world\_value\_survey} dataset. How many correspondents
have a religion type of ``Muslim''? What is the proportion of that to
...
\CommentTok{\#get a frequency table}
\FunctionTok{data.frame}\NormalTok{(}\FunctionTok{table}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{religion))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#get a frequency table along with proportion}
\NormalTok{world\_value }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{count}\NormalTok{(religion) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{pct =} \FunctionTok{round}\NormalTok{(n }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(n) }\SpecialCharTok{*} \DecValTok{100}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Same as we've done in Lab 2, we can very easily get the frequency of
categorical variable by using \texttt{table()},
\texttt{data.frame(table())} would present the frequency in a dataframe
table format. But to get the proportion along with the frequency, we use
\texttt{count()} and then \texttt{mutate()} a new column called
\texttt{pct} for the percentage of each count of category to the total
sample. The answer is: 30,081 respondents report themselves as Muslim in
the religion, which accounts for 19.7\% of the total sample.

\section{Question 8}\label{question-8}

Write code to create boxplots of variable \emph{Extractive\_Job},
\emph{Productive\_Job}, \emph{Professional}, \emph{Routine\_Job} using
the \emph{Census\_2021\_Wards} dataset.~ Based on your plotst, which
variable shows the most \textbf{positively skewed} distribution?
(Recall: a positively skewed boxplot has a longer upper whisker and the
median positioned closer to the lower quartile.) How to interpret the
result?
...
\CommentTok{\# boxplot comparing serveral variables}
\FunctionTok{boxplot}\NormalTok{(}
\NormalTok{  wards[, }\FunctionTok{c}\NormalTok{(}\StringTok{"Extractive\_Job"}\NormalTok{,}
            \StringTok{"Productive\_Job"}\NormalTok{,}
            \StringTok{"Professional"}\NormalTok{,}
            \StringTok{"Routine\_Job"}\NormalTok{)],}
  \AttributeTok{horizontal =} \ConstantTok{TRUE}\NormalTok{,}
  \AttributeTok{main =} \StringTok{"Occupation"}\NormalTok{,}
  \AttributeTok{xlab =} \StringTok{\textquotesingle{}Percentage\textquotesingle{}}\NormalTok{,}
  \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"gold"}\NormalTok{, }\StringTok{"pink"}\NormalTok{, }\StringTok{"lightblue"}\NormalTok{, }\StringTok{"lightgreen"}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-9-1.pdf}}

The
\texttt{wards{[},\ c("Extractive\_Job",\ "Productive\_Job",\ "Professional",\ "Routine\_Job"){]}}
selected these four variable from the large raw dataframe.
\texttt{boxplot()} then only plot boxplots for these four variables, and
\texttt{col\ =\ c()} assigned four colors to each of these four variable
with the same order of the variables. Since all these four variables are
in percentage format so they are comparable. The
\texttt{Extractive\_Job} variable shows the strongest positive skewness
as the median (black line in the box) positioned closer to the lower
quartile. This can be interpret as most wards have relatively low
proportions of extractive employment. In addition, there are more
outliers on the right-hand side of the distribution, suggesting that
while most areas have relatively low proportions of extractive
employment, a small number of areas have markedly higher percentages.
This creates a long upper tail and indicates that extractive-sector
employment is highly concentrated in a limited number of locations
rather than being evenly distributed across areas. The answer is
extractive\_job. But try to interpret other vairables from their
boxplots by yourself.

\section{Question 9}\label{question-9}

Write code to compute mean and standard deviation of variables
\emph{Disabled} and \emph{Poor\_Health} in the
\emph{Census\_2021\_Wards.cs}v dataset in R. Compare and interpret the
...
\CommentTok{\# Compute results for Disabled }
\CommentTok{\#mean value}
\FunctionTok{mean}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Disabled)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#standard deviation}
\FunctionTok{sd}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Disabled)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Compute results for Poor\_Health}
\CommentTok{\#mean value}
\FunctionTok{mean}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Poor\_Health)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#standard deviation}
\FunctionTok{sd}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Poor\_Health)}
\end{Highlighting}
\end{Shaded}
...
The mean proportion of residents reporting disability across wards is
\textbf{17.96\%} (SD = \textbf{4.00}), while the mean proportion
reporting poor health is slightly higher at \textbf{18.25\%} (SD =
\textbf{4.25}). This indicates that, on average, the prevalence of poor
health is marginally greater than that of disability, although the
difference between the two measures is small. The standard deviations
are also similar, suggesting comparable levels of variation between
wards for both indicators, with poor health exhibiting slightly greater
dispersion. Overall, the results imply that the two variables follow
broadly similar distributions across areas in terms of both central
tendency and variability, indicating a consistent spatial pattern in the
prevalence of disability and poor health at the ward level.

\section{Question 10}\label{question-10}

Write code to compare the life satisfaction (variable:
life\_satisfaction) between different marital status (variable:
marital\_status) using the \emph{world\_value\_survey.csv} dataset using
boxplots. Interpret the findings?
...
\FunctionTok{table}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{marital\_status)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boxplot}\NormalTok{(life\_satisfaction }\SpecialCharTok{\textasciitilde{}}\NormalTok{ marital\_status, }
        \AttributeTok{data =}\NormalTok{ world\_value)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-11-1.pdf}}

As we did in Lab 2, we can create boxplot between categorical variable
to compare a numerical variable. Here it is the
\texttt{life\_satisfaction\ \textasciitilde{}\ marital\_status}. Using
the knowlege of boxplot, we can learn that :

\begin{itemize}
\item
  The \textbf{median life satisfaction} (thick black line in each box)
  is highest for Married and Cohabiting individuals (around 8), and
  slightly lower for Divorced, Separated, Single/Never married, and
  Widowed (around 7). This suggests that married or cohabiting people
  report higher life satisfaction on average.
\item
  The \textbf{interquartile range (IQR)} --- the height of the box ---
  is similar across groups (roughly 5--8), indicating moderate variation
  in life satisfaction within each marital status. Divorced and
  Separated groups show slightly wider spread toward lower values,
  indicating more variation among individuals.
\item
  Most boxes are roughly \textbf{symmetric}, but Divorced and Separated
  groups have medians closer to the upper quartile, suggesting a slight
  \textbf{negative skew} (more respondents at lower satisfaction
  scores).
\item
  Small dots below the whiskers indicate \textbf{outliers} with
  unusually low life satisfaction. Outliers show that a few individuals
  in each group report very low life satisfaction compared to the
  majority.
\end{itemize}

Overall, Marriage and cohabitation are associated with \textbf{higher
life satisfaction}. Being divorced or separated is associated with
\textbf{slightly lower satisfaction and greater variability}. Single or
widowed individuals are in the middle range.

\section{Question 11}\label{question-11}

The age in the real world is always a normal distribution. We assume
that the age in the World Value Survey is also normal distribution.
Write code to calculate the \textbf{mean} and \textbf{standard
deviation} of age using the \emph{world\_value\_survey.csv} dataset.
Based on your output, use the 68-95-97.5 rule to interpret the result.
...
\CommentTok{\#mean value, na.rm=TRUE to remove the NA value in variable}
\FunctionTok{mean}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{age,}\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#standard deviation}
\FunctionTok{sd}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{age,}\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The mean age represents the central tendency of the sample, indicating
the average age of individuals included in the dataset. The mean age of
the sample is \textbf{45.4 years}, suggesting that the average
respondent is in their mid-forties. The standard deviation (\textbf{17.2
years}) indicates the variability in age within the sample, with a
relatively large value implying substantial heterogeneity in ages.
Assuming an approximately normal distribution, about \textbf{68\%} of
respondents are expected to fall within one standard deviation of the
mean (approximately \textbf{28.2 to 62.6 years}), about \textbf{95\%}
within two standard deviations (approximately \textbf{11.0 to 79.8
years}), and about \textbf{99.7\%} within three standard deviations
(approximately \textbf{−6.2 to 97.0 years}). Since negative ages are not
meaningful, the lower bound would effectively be truncated at
\textbf{zero}, but the calculation still illustrates the wide spread of
ages in the dataset.

\section{Question 12}\label{question-12}

Write code to compute the \textbf{standard error} \textbf{of the mean}
\textbf{proportion} for work status as \emph{Housewife}. (variable:
work\_status, category: Housewife) across all observations in the World
Value Survey dataset (\emph{world\_value\_survey.csv}). What is the
approximate result?
...
\NormalTok{n }\OtherTok{=} \FunctionTok{nrow}\NormalTok{(world\_value)}
\NormalTok{n}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Sample mean proportion}
\NormalTok{p\_hat }\OtherTok{=} \FunctionTok{mean}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{work\_status }\SpecialCharTok{==} \StringTok{"Housewife"}\NormalTok{,}\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{p\_hat}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Standard Error for a proportion}
\NormalTok{SE\_p }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(p\_hat }\SpecialCharTok{*}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ p\_hat) }\SpecialCharTok{/}\NormalTok{ n)}
\NormalTok{SE\_p}
\end{Highlighting}
\end{Shaded}
...
Therefore, we need first get the \texttt{n} as the sample size, here it
is \textbf{153009}. We then need the p-hat which is the mean proportion
of our sample: 10.3\%. This indicates that approximately
\textbf{10.30\%} of respondents have a work status classified as
\emph{Housewife}. Because our sample size is very large, the standard
error is extremely small (less than 0.001).

\section{Question 13}\label{question-13}

Write code to compute the mean value and 95\% confidence interval of the
life\_satisfaction variable in the World Value Survey
(\emph{world\_value\_survey.csv}).
...
\CommentTok{\#CI windows}
\NormalTok{lifeSatis }\OtherTok{\textless{}{-}} \FunctionTok{na.omit}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{life\_satisfaction)}
\CommentTok{\#sample size}
\NormalTok{n }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(lifeSatis)}
\NormalTok{n}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{m }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(lifeSatis)}
\NormalTok{m}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#sample standard deviation}
\NormalTok{std }\OtherTok{\textless{}{-}} \FunctionTok{sd}\NormalTok{(lifeSatis)}
\NormalTok{std}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#standard error}
\NormalTok{se }\OtherTok{\textless{}{-}}\NormalTok{ std}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(n)}
\NormalTok{se}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{error }\OtherTok{\textless{}{-}} \FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{)}\SpecialCharTok{*}\NormalTok{se}

\CommentTok{\#CI window}
\NormalTok{lower.bound }\OtherTok{\textless{}{-}}\NormalTok{ m}\SpecialCharTok{{-}}\NormalTok{error}
\NormalTok{upper.bound }\OtherTok{\textless{}{-}}\NormalTok{ m}\SpecialCharTok{+}\NormalTok{error}
...
The CI of numerical variable should be calculated as
\includegraphics[width=2.84375in,height=0.29167in]{labs/images/clipboard-3403969333.png}.
It is easy to get \textbf{sample mean m}, but for the Standard
Error\includegraphics[width=1.03125in,height=\textheight,keepaspectratio]{labs/images/clipboard-1671976609.png}
, we need \textbf{standard deviation sd} and \textbf{sample size n}.

Therefore, in above code, we get \textbf{sample size n = 152372},
\textbf{sample mean m = 7.22, sample standard deviation sd = 2.15}, then
the \textbf{Standard Error (SE)} can be calculated by
\texttt{error\ \textless{}-\ qnorm(0.975)*std/sqrt(n)} as the \textbf{se
= 0.0055}.

The CI window are computed by
\texttt{sample\ mean\ +/-\ qnorm(0.975)*Standard\ Error} .

We can interpret the outputs as: The mean life satisfaction score in the
sample is \textbf{7.22} (SD = \textbf{2.15}) based on \textbf{152,372}
respondents. The relatively large standard deviation indicates
substantial variation in life satisfaction across individuals. However,
the standard error is very small (\textbf{0.0055}) due to the large
sample size, resulting in a narrow 95\% confidence interval ranging from
\textbf{7.214 to 7.236}. This suggests that the true population mean
life satisfaction is very likely to lie within this interval, indicating
a high level of precision in the estimate.

\section{Question 14}\label{question-14}

Write code to produce a cross-tabulation of settlement type (variable:
settlement\_type) by how they feel related to the world (variable:
relate\_to\_world) using the \emph{world\_value\_survey.csv} dataset.
According to your table,~

Based on your results, which type of settlement has the highest mean
proportion of feeling very close to the world?
...
\CommentTok{\# create a cross tabulation table }
\NormalTok{tab }\OtherTok{=} \FunctionTok{table}\NormalTok{(world\_value}\SpecialCharTok{$}\NormalTok{settlement\_type, world\_value}\SpecialCharTok{$}\NormalTok{relate\_to\_world)}

\CommentTok{\# convert the table into proportion format by each row as 100\%}
\FunctionTok{round}\NormalTok{(}\FunctionTok{prop.table}\NormalTok{(tab, }\AttributeTok{margin =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{*} \DecValTok{100}\NormalTok{, }\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
  ...
  Village (pop. 5-20k)         32.1             17.4           33.1       17.5
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#make the table as a dataframe for further handling}
\NormalTok{result }\OtherTok{=} \FunctionTok{round}\NormalTok{(}\FunctionTok{prop.table}\NormalTok{(tab, }\AttributeTok{margin =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{*} \DecValTok{100}\NormalTok{, }\DecValTok{1}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.data.frame.matrix}\NormalTok{()}
\NormalTok{result}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#return the max value in column Do not trust at all}
\NormalTok{result }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{slice}\NormalTok{(}\FunctionTok{which.max}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{Very close}\StringTok{\textasciigrave{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
As we did in Lab 3, we use \texttt{table()} for a cross tabulation of
two categorical variables to get the frequency table. Then convert the
frequency table into a proportion one in \texttt{prop.table()}:
\texttt{margin\ =\ 1} when we need the row sum up to 100\%;
\texttt{margin\ =\ 2} when we wish the column sum up as 100\%. To get
the max or min value or any further data analysis, we convert the
proportion table as a dataframe by
\texttt{\%\textgreater{}\%\ as.data.frame.matrix()}, rounded all the
digits to only 1 by using \texttt{round(\ ,.1)} . Same as Q1.4, we use
\texttt{slice(which.max())} to return the row with the highest value in
Very close. Noted: for column names with space like this one, we use ``
of the column name when we need to use it. Therefore here use see
\texttt{Very\ close} are quoted in '\,'. The answer is Hamlet with
population less than 5k shows highest proportion of feel themselves very
close to the world in our survey sample. Are you surprised by this
result?

\section{Question 15}\label{question-15}

What is the Standard Error of samples? With the increase of sample, how
will Standard Error change?

Solution

The \textbf{Standard Error (SE)} measures how much a sample statistic,
such as a mean or proportion, is likely to vary from one sample to
another. It provides an estimate of the precision of the sample
statistic. For example, the SE of a mean is calculated by dividing the
standard deviation of the sample by the square root of the sample
size:\includegraphics[width=1.15625in,height=\textheight,keepaspectratio]{labs/images/clipboard-1671976609.png},
while the SE of a proportion is calculated using the formula
\includegraphics[width=1.58333in,height=\textheight,keepaspectratio]{labs/images/clipboard-845536362.png}

As the \textbf{sample size increases}, the standard error decreases.
This is because larger samples provide more information about the
population, making the estimate more precise. Therefore, increasing the
number of observations reduces sampling variability and gives greater
confidence that the sample statistic is close to the true population
value.

\section{Question 16}\label{question-16}

You are required to write code to combine two datasets, \emph{LAD
December 2021 EW BUC.geojson} and \emph{Wards\_2021\_Districts.csv},
using a \textbf{left join}. Both datasets contain a common variable
called \emph{LAD21CD,} which is the unique code assigned by the Office
for National Statistics (ONS) to each local authority district in the
2021 Census. This variable should be used to match the two datasets
correctly. After joining the datasets, use the combined data to create a
map showing the percentage of self-employed people at the district level
across the UK.
...
\NormalTok{joined\_map }\OtherTok{\textless{}{-}} \FunctionTok{left\_join}\NormalTok{(district\_map, districts, }\AttributeTok{by =} \StringTok{"LAD21CD"}\NormalTok{)}

\CommentTok{\# Map variable}
\NormalTok{map }\OtherTok{\textless{}{-}} 
  \FunctionTok{tm\_shape}\NormalTok{(joined\_map) }\SpecialCharTok{+}
  \FunctionTok{tm\_polygons}\NormalTok{(}
    \AttributeTok{fill =} \StringTok{"Self\_Employed"}\NormalTok{,}
    \AttributeTok{fill.scale =} \FunctionTok{tm\_scale\_continuous}\NormalTok{(}\AttributeTok{values =} \StringTok{"Blues"}\NormalTok{)}
\NormalTok{  )}


\CommentTok{\# Display map}
\NormalTok{map}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-16-1.pdf}}

\subsubsection{}\label{section}

\section{Question 17}\label{question-17}

Write code to create a scatterplot showing the relationship between the
variables \emph{Density} and \emph{Born\_in\_UK} using the
\emph{Census\_2021\_Districts.csv} dataset. Make sure that
\emph{Density} is shown on the x-axis and Born\_in\_UK is shown on the
y-axis.
...
\NormalTok{districts }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Density, }\AttributeTok{y =}\NormalTok{ Born\_in\_UK), }\AttributeTok{colour =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Density"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Born in UK"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-17-1.pdf}}

\textbf{IMPORTANT:} Notice in the above line of code, ``x =
\emph{Density}, y = \emph{Born\_in\_UK}'' , this is where you specify
what goes on the x and y axis respectively. If the code was ``x =
\emph{Born\_in\_UK}, y = \emph{Density}'' then you would be plotting
\emph{Born\_in\_UK} on the x-axis with \emph{Density} on the y-axis.

\section{Question 18}\label{question-18}

Write code to calculate the appropriate correlation coefficient between
the variables \emph{Mean\_Age} and \emph{Poor\_Health} using the
Census\_2021\_Wards.csv dataset. Please also interpret what the
resulting value mean.

Solution

\textbf{Remember the steps:} Check whether the variables are numerical
or categorical \textgreater{} If numerical check for skewness
\textgreater{} If both variables have minor or no skew then choose
Pearson's r, otherwise it's Spearman's Rank correlation coefficient.
...
\CommentTok{\# Check whether the data is of numerical or categorical type}
\FunctionTok{view}\NormalTok{(wards)}
\end{Highlighting}
\end{Shaded}

Viewing the table it is evident that both variables are of numeric type.
In the case the correlation coefficient is either Pearson's r or
Speakman's Rank

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calculate skewness of variables}
\DocumentationTok{\#\# Poor\_Health}
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ wards}\SpecialCharTok{$}\NormalTok{Poor\_Health}
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ x[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)]}
\FunctionTok{mean}\NormalTok{((x }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(x))}\SpecialCharTok{\^{}}\DecValTok{3}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sd}\NormalTok{(x)}\SpecialCharTok{\^{}}\DecValTok{3}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# Mean\_Age}
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ wards}\SpecialCharTok{$}\NormalTok{Mean\_Age}
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ x[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)]}
\FunctionTok{mean}\NormalTok{((x }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(x))}\SpecialCharTok{\^{}}\DecValTok{3}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sd}\NormalTok{(x)}\SpecialCharTok{\^{}}\DecValTok{3}
\end{Highlighting}
\end{Shaded}
...
There is~moderate skew in \emph{Poor\_Health} and minor skew in
\emph{Mean\_Age}. Because \textbf{both} (not just one) numerical
variables do not have minor or no skew, Spearman's Rank correlation
would be the appropriate measure to use here. Had both variables have
minor or no skew, \textbf{ONLY THEN} would Pearson's r correlation be
appropriate.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor}\NormalTok{(wards}\SpecialCharTok{$}\NormalTok{Poor\_Health,}
\NormalTok{    wards}\SpecialCharTok{$}\NormalTok{Mean\_Age,}
    \AttributeTok{method =} \StringTok{"spearman"}\NormalTok{,}
    \AttributeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
NOTE: While not asked as part of the question, a Spearman's value of 0.2
would be interpreted as a weak association between \emph{Poor\_Health}
and \emph{Mean\_Age}. More on interpreting the outputs on correlation
can be found in Week 4 lab.

\section{Question 19}\label{question-19}

Write code to create a barplot showing the relationship between the
variables \emph{marital\_status} and \emph{happiness} using the
\emph{world\_value\_survey.csv} dataset. What pattern(s) can be
observed?
...
\NormalTok{world\_value }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_bar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ marital\_status, }\AttributeTok{fill =}\NormalTok{ happiness)) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}
    \AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{45}\NormalTok{, }\AttributeTok{hjust =} \DecValTok{1}\NormalTok{)}
\NormalTok{  )}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-21-1.pdf}}

Married people show the strongest association with being happy.

You can also see that most people report being happy regardless of
marital status.

\section{Question 20}\label{question-20}

When trying to understand the skewness of a variable, why is it
important to use both visualisations and skewness metrics?

Solution

Visualisations provide qualitative insight into the distribution's
shape, structure, and outliers, while skewness metrics provide
quantitative, objective measurement of asymmetry. Using both ensures
accurate, reliable, and complete understanding of the variable's
distribution, reducing the risk of misinterpretation.

\section{Question 21}\label{question-21}

Write code to calculate the Gini coefficient between the variables
\emph{Poor\_Health} using the Census\_2021\_Wards.csv dataset. Be sure
to not include an NA values. Please also interpret what the resulting
value mean.
...
\CommentTok{\# Compute Gini value}
\FunctionTok{Gini}\NormalTok{( wards}\SpecialCharTok{$}\NormalTok{Poor\_Health, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{ )}
\end{Highlighting}
\end{Shaded}
...
\textbf{NOTICE:} In the above code ``na.rm = TRUE'' removes NA values
before the Gini value is computed. This is important as these values can
lead to misleading or incorrect results. Also remember that Gini
coefficient measures the level of inequality in a distribution of a
variable and when interpreting results, values range from 0 (perfect
equality) to 1 (perfect inequality).

A Gini coefficient of 0.131 for poor health indicates low spatial
inequality in the distribution of poor health across wards.

\section{Question 22}\label{question-22}

Compute the Spearman's rank correlation coefficient between the numeric
variable \emph{Self\_Employed} (equivalent net pay) and the other
numeric variables in the Census\_2021\_Wards.csv dataset, along with
their associated p-values. Based on your results, interpret what do they
mean in the context of the association between \emph{Self\_Employed} and
\emph{Married} and its p-value?

Solution

In the code below, the only things that will be changing is the
information in PART 1 and PART 2. So if you were asked a similar
question in the exam, you would copy the entire piece of code but only
change that data to be loaded, the variable of interest, and the
correlation method you want to use. The other part of the code should
remain unchanged and it's not expected that you will have to memorise it
or write it from scratch.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# PART 1: Load the data}
\NormalTok{data }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"Census\_2021\_Wards.csv"}\NormalTok{,}\AttributeTok{stringsAsFactors =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# PART 2: Input your variable of interest and correlation method}
\CommentTok{\# The variable that you want as your main achor}
\NormalTok{variable\_of\_interest }\OtherTok{=} \StringTok{"Self\_Employed"}
...
\CommentTok{\# PART 3: Compute the correlation coefficients and associated p{-}values}
...
\NormalTok{  data }\SpecialCharTok{\%\textgreater{}\%}
  \CommentTok{\# keep only numeric columns}
  \FunctionTok{select}\NormalTok{(}\FunctionTok{where}\NormalTok{(is.numeric)) }\SpecialCharTok{\%\textgreater{}\%}
  \CommentTok{\# pivot all numeric columns except the anchor into long format}
...
\NormalTok{  ) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{summarise}\NormalTok{(}
    \AttributeTok{rho =} \FunctionTok{cor}\NormalTok{(x, .data[[variable\_of\_interest]],}
              \AttributeTok{method =}\NormalTok{ correlation\_method,}
              \AttributeTok{use =} \StringTok{"pairwise.complete.obs"}\NormalTok{),}
    \AttributeTok{p\_value =} \FunctionTok{cor.test}\NormalTok{(x, .data[[variable\_of\_interest]],}
                       \AttributeTok{method =}\NormalTok{ correlation\_method,}
                       \AttributeTok{exact =} \ConstantTok{FALSE}\NormalTok{)}\SpecialCharTok{$}\NormalTok{p.value,}
...
\CommentTok{\# PART 4: View the output}
\FunctionTok{View}\NormalTok{(correlations\_with\_pvalue)}
\end{Highlighting}
\end{Shaded}

\textbf{NOTICE:} When you receive the output table you can columns in
ascending or descending order by clicking the name of the column, just
like you would when using Microsoft Excel.

The Spearman's rank correlation coefficient between
\emph{Self\_Employed} and \emph{Married} is 0.49, indicating a moderate
positive association. This suggests that wards with higher proportions
of self-employed individuals tend to also have higher proportions of
married residents. The p-value is less than 0.05, indicating that this
relationship is statistically significant and unlikely to have occurred
by chance. This finding suggests a meaningful association between
self-employment and marital status at the ward level, potentially
reflecting broader socio-economic and demographic patterns.

\section{Question 23}\label{question-23}

Write code to plot the Lorenz curve for the variable \emph{Density}
using the \emph{Census\_2021\_Wards.csv} dataset. Please interpret what
the resulting value mean.
...
\CommentTok{\# Draw the Lorenz curve}
\NormalTok{wards }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{pull}\NormalTok{(Density) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{Lc}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{plot}\NormalTok{(}
    \AttributeTok{xlab =} \StringTok{"Cumulative proportion of wards (p)"}\NormalTok{,}
    \AttributeTok{ylab =} \StringTok{"Cumulative proportion of Density (L(p))"}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/Mock-exam_B_Solutions_files/figure-pdf/unnamed-chunk-24-1.pdf}}

The Lorenz curve for Density shows a clear deviation from the line of
perfect equality, indicating that density is unevenly distributed across
wards. A relatively small proportion of wards accounts for a large share
of total density, while most wards have comparatively low density. This
pattern most likely reflects strong spatial concentration, consistent
with urban areas having much higher density than suburban or rural
wards.

\section{Question 24}\label{question-24}

Write code to compute the skewness for the variable \emph{Density} using
the \emph{Census\_2021\_Wards.csv} dataset. Please interpret what the
resulting value mean.

Solution

Remember, while most skewness values for variables tend to fall within
the range of-1 to +1, values can exist outside this range. Those out of
this range are can be classed as highly skewed, or in some cases,
extremely skewed.
...
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ wards}\SpecialCharTok{$}\NormalTok{Density}
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ x[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)]}
\FunctionTok{mean}\NormalTok{((x }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(x))}\SpecialCharTok{\^{}}\DecValTok{3}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sd}\NormalTok{(x)}\SpecialCharTok{\^{}}\DecValTok{3}
\end{Highlighting}
\end{Shaded}
...
The skewness value of 2.38 indicates that \emph{Density} is highly
positively skewed. This means that most wards have relatively low
population density, while a small number of wards have extremely high
density. This most likely reflects the spatial structure of urban
systems, where dense urban centres contrast with less dense suburban and
rural areas.

\section{Question 25}\label{question-25}

In addition to the p-value, we can also calculate the confidence
intervals for the correlation between two variables. Why are confidence
intervals useful in this case?

Solution

Confidence intervals are useful because they show the range of values
the true correlation is likely to fall within, rather than just saying
whether a relationship exists. as in the case of the p-value.
