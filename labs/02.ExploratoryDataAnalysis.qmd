---
title: "02.Exploratory Data Analysis - UK Election"
author: "Zi Ye"
date: "`r Sys.Date()`"
...
-   Open a new R Script for your Week 2 work, rename it as Week2.R and save it in your newly creately Week 2 folder, under M drive -\> ENVS162 folder. This is exactly the step we did in Week 1, and we will do this every week to Week 5.

-   Check whether there is any previous left dataframe in your RStudio in the upper-right side Environment pane. You can always use the ![](images/clipboard-4263234033.png) to clear all the dataframes in your environment and make it all clean. For the same aim, you can run the below code:
...
The `scales` package provides the command `percentage( )` which we can easily convert a number to its percentage format, and also specify how many digits we wish to keep.

If you are using a University PC Teaching Centre computer the `scales` package should come pre-installed.
...
In 2024 the UK held a general election. Download the file ***uk_constituencies_2024.csv*** from our [Canvas module page Week 2](https://canvas.liverpool.ac.uk/courses/85565). Read in the dataset exactly in the same way as we did in Week 1 - insert the below code line and run it:
...
```{r}
plot(density(pc_data$valid_votes), main = "Valid votes")
```

```{r}
plot(density(pc_data$pct_White_British), main = "% of White British in Constituency", col="salmon", lwd=2)
```
...
```{r}
boxplot(pc_data$valid_votes,horizontal=TRUE, main = "Valid votes", xlab='Votes', col = "gold")
```

```{r}
boxplot(pc_data$pct_White_British,horizontal=TRUE, main = "% of White British", xlab='Percentage', col = "hotpink")
```
...
```{r}
boxplot(median_house_price ~ first_party, data = pc_data, las = 2, xlab="",ylab="",main="Compare constituency median house price vs. first party")
```

```{r}
boxplot(pct_White_British ~ mp_gender, data = pc_data,col=c("red", "yellow3"), horizontal = TRUE)
```
...
```{r}
pc_map <- st_read("uk_constituencies_2024.gpkg") #read the boundaries as a spatial dataset by using the st_read() command from library(sf)
```
...
tm_shape(pc_map_new) + #map a spatial data
  tm_polygons(fill = "first_party", 
              palette="Set3")  #map it as polygons, use different colors by first_party
```
...
**Task 4** Write codes to summary the counts of MPs in each gender (this is in the `mp_gender`variable), presenting the table with percentage and showing the barplot.
...
**Task 5** Cross tabulation `region_name` to `mp_gender` by using the newly created `pc_map_new` dataset, which joined by the constituency boundary and election dataset.

```{r}
table(pc_map_new$region_name, pc_map_new$mp_gender)
```

**Task 6** Compare between `crime_rate` with the `pct_social_rented` in constituency election dataset. What pattern you can learn from your boxplot?
