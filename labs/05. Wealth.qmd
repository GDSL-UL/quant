...
## **5.1 Identifying the outcome(s) of interest**
...
```{r}
correlations_with_net_pay <-
APS %>%
  select(where(is.numeric)) %>%
  select(-idref, -Household_ID, -Extended_Family_ID, -Family_ID) %>%
  drop_na() %>%
  ...
  arrange(desc(abs(Net_Pay)))

View(correlations_with_net_pay)
```
...
To check whether a relationship is significant we can examine their p value and significance value.

```{r}
correlations_with_net_pay_pvalue <-
APS %>%
  select(where(is.numeric)) %>%
  select(-idref, -Household_ID, -Extended_Family_ID, -Family_ID) %>%
  ...
  group_by(Variable) %>%
  summarise(
    rho = cor(x, Net_Pay, method = "spearman", use = "pairwise.complete.obs"),
    p_value = cor.test(
      x, Net_Pay,
      method = "spearman",
      exact = FALSE
...
View(correlations_with_net_pay_pvalue)
```
...
```{r}
correlations_with_net_pay_pvalue_ci <-
APS %>%
  select(where(is.numeric)) %>%
  select(-idref, -Household_ID, -Extended_Family_ID, -Family_ID) %>%
  pivot_longer(-Net_Pay, names_to = "Variable", values_to = "x") %>%
  group_by(Variable) %>%
  summarise(
    n = sum(is.finite(x) & is.finite(Net_Pay)),
    rho = cor(x, Net_Pay, method = "spearman", use = "pairwise.complete.obs"),
    p_value = cor.test(
      x, Net_Pay,
      method = "spearman",
    ...
    conf_low = tanh(atanh(rho) - qnorm(0.975) / sqrt(n - 3)),
    conf_high = tanh(atanh(rho) + qnorm(0.975) / sqrt(n - 3)),
    sig = p_value <= 0.05,
    .groups = "drop"
  ) %>%
  filter(Variable != "Net_Pay") %>%
...
View(correlations_with_net_pay_pvalue_ci)
```
